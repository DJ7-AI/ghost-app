...
module.exports = async function(req, res) {
  ...
  messages.slice(-20).forEach(m => {
    if (m.content) openaiMessages.push({
  ...
  const payload = JSON.stringify({
    model: 'gpt-4o-mini', // cheap, fast, great for roasts
    max_tokens: Math.min(maxTokens, 500),
    messages: openaiMessages,
    temperature: 0.9, // slightly higher = more personality
  });
  ...
  return new Promise((resolve) => {
    ...
    const request = https.request(options, response => {
      ...
      response.on('end', () => {
        console.log('[Ghost] OpenAI status:', response.statusCode);
        if (response.statusCode !== 200) {
          console.error('[Ghost] OpenAI error:', data);
